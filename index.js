...
/* ================= DOMAIN / PROXY / UA ================= */
const DOMAINS_MAP = {
  es: "https://penidadivecenter.es",
};

const PROXIES = {
  es: process.env.BRD_PROXY_ES, // optional
};

const USER_AGENTS = {
  es: "PenidaDiveCenter-ES-CacheWarmer/1.0",
};
...
function extractCfEdge(cfRay) {
  if (typeof cfRay === "string" && cfRay.includes("-")) {
    return cfRay.split("-").pop(); // FRA, MAD, AMS, etc
  }
  return "N/A";
}
    ...
    await Promise.all(
      batch.map(async (url) => {
        ...
        try {
          ...
          logger.log({
            country: edge, // ✅ EDGE MASUK KE COUNTRY
            url,
            status: res.status,
            cfCache,
            lsCache,
          ...
          // 🔥 FINAL RULE: LiteSpeed = kebenaran
